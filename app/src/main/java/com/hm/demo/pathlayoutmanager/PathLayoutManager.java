package com.hm.demo.pathlayoutmanager;
...
import com.hm.demo.pathlayoutmanager.keyframes.Keyframes;
import com.hm.demo.pathlayoutmanager.keyframes.PosTan;
...
//            recyclerView.setItemAnimator(null);
    ...
    private void onLayout(RecyclerView.Recycler recycler, List<PosTan> needLayoutItems) {
        int x, y;
        View item;
        float fraction = getScrollOffset() * 1f / getItemLength();
        ...
        for (PosTan tmp : needLayoutItems) {
            ...
            if (mItemChangeListener != null) {
                mItemChangeListener.itemChange(item, tmp.getChildAngle(), fraction);
            }
        }

        if (mItemChangeListener != null) {
            mItemChangeListener.scrollChange(fraction);
        }
    }
    ...
    private void updateOffsetX(float offsetX) {
        ...
        int pathLength = mKeyframes.getPathLength();
        int itemLength = getItemLength();
        Log.i(TAG, "updateOffsetX itemLength = " + itemLength + " pathLength = " + pathLength);
        if (isSatisfiedLoopScroll(pathLength, itemLength)) {
...
//            mItemOffset改变后要刷新mItemCountInScreen
    ...
    public interface ItemChangeListener {
        void itemChange(View item, float angle, float fraction);

        void scrollChange(float fraction);
    }
}