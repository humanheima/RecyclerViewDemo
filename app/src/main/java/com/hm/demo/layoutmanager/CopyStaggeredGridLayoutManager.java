 ...
 */
public class CopyStaggeredGridLayoutManager extends RecyclerView.LayoutManager implements
     ...
     */
    @SuppressWarnings("unused")
    public CopyStaggeredGridLayoutManager(Context context, AttributeSet attrs, int defStyleAttr,
            int defStyleRes) {
     ...
     */
    public CopyStaggeredGridLayoutManager(int spanCount, int orientation) {
...
//                : shouldMeasureChild(child, widthSpec, heightSpec, lp);
    ...
    // TODO: 2024/3/31 对应  AdapterHelper.UpdateOp.ADD
    static final int ADD = 1;

    // TODO: 2024/3/31 对应  AdapterHelper.UpdateOp.REMOVE
    static final int REMOVE = 1 << 1;

    // TODO: 2024/3/31 对应  AdapterHelper.UpdateOp.UPDATE
    static final int UPDATE = 1 << 2;

    // TODO: 2024/3/31 对应  AdapterHelper.UpdateOp.MOVE
    ...
    public void onItemsRemoved(RecyclerView recyclerView, int positionStart, int itemCount) {
        ...
        handleUpdate(positionStart, itemCount, REMOVE);
    }
    ...
    public void onItemsAdded(RecyclerView recyclerView, int positionStart, int itemCount) {
        ...
        handleUpdate(positionStart, itemCount, ADD);
    }
    ...
    public void onItemsUpdated(RecyclerView recyclerView, int positionStart, int itemCount,
        ...
        handleUpdate(positionStart, itemCount, UPDATE);
    }
    ...
    private int fill(RecyclerView.Recycler recycler, LayoutState layoutState,
        ...
        while (layoutState.hasMore(state)
            ...
            if (assignSpan) {
                ...
                currentSpan = lp.mFullSpan ? mSpans[0] : getNextSpanWithPosition(layoutState, position);
    ...
    /**
     * Finds the span for the next view.
     * 在只有两列的情况下，按照奇偶数来找到下一个span
     */
    private Span getNextSpanWithPosition(LayoutState layoutState, int position) {
        final boolean preferLastSpan = preferLastSpan(layoutState.mLayoutDirection);
        final int startIndex, endIndex, diff;
        if (preferLastSpan) {
            startIndex = mSpanCount - 1;
        ...
        if (layoutState.mLayoutDirection == LayoutState.LAYOUT_END) {
            boolean evenNumber = position % 2 == 0;
            Span min = null;
            final Span other;
            if (evenNumber) {
                other = mSpans[0];
            ...
            min = other;
            //Log.i(TAG, "getNextSpanWithPosition: position = " + position + " ，min.getEndLine() = " + min.getEndLine());
            Log.i(TAG, "getNextSpanWithPosition: position = " + position);
            return min;
        } else {
            Span max = null;
            int maxLine = Integer.MIN_VALUE;
