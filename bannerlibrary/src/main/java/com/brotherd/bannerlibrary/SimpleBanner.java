...
public class SimpleBanner extends RelativeLayout implements ViewPager.OnPageChangeListener {
    ...
    private void initTypedArray(Context context, AttributeSet attrs) {
        ...
        mTipTextSize = typedArray.getDimensionPixelSize(R.styleable.SimpleBanner_banner_tipTextSize, mTipTextSize);
        mPointGravity = typedArray.getInt(R.styleable.SimpleBanner_banner_point_gravity, mPointGravity);
        int ordinal = typedArray.getInt(R.styleable.SimpleBanner_banner_transitionEffect, TransitionEffect.Default.ordinal());
    ...
    /**
     * @param titles    轮播标题
     * @param imageUrls 轮播图片地址
     */
    private void initPlay(List<String> titles, List imageUrls) {
        if ((titles == null || titles.size() <= 0) || (imageUrls == null || imageUrls.size() <= 0)) {
            throw new IllegalStateException("when initPlay(List<String> titles, List<?> imageUrls) titles and imageUrls can not be null");
        }
        if (imageLoader == null) {
